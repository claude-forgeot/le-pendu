...
def play_lose_sequence(secret_word):
    """Loss: Highscore check, then Overlay noir transparent + lose_infinite.mp3."""
    global current_total_score
    pygame.mixer.music.stop()
    
    # 1. Vérification du Highscore AVANT l'écran de Game Over
    if score_manager.check_if_highscore(current_total_score, category="infinite"):
        get_name_input(current_total_score)
    ...
    while True:
        ...
        t_score = fonts["info"].render(f"SCORE FINAL : {current_total_score}", True, constants.GOLD)
        t2 = fonts["info"].render(f"Le mot était : {secret_word}", True, constants.WHITE)
        tip = fonts["small"].render("L'infini vous a consumé...", True, constants.WHITE)
...
def draw_interface(state, secret, mouse_pos):
    ...
    surf_score = fonts["info"].render(f"SCORE: {current_total_score}", True, constants.WHITE)
...
def main():
    ...
    while True:
        ...
        if game_state["status"] == "won":
            # On ajoute les points du mot trouvé au score total
            current_total_score += score_manager.calculate_score(game_state)
            # Auto-restart immédiat en mode Infinite
            game_state, secret_word = initialize_game(reset_score=False)
