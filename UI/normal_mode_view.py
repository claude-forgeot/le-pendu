...
Win: winnormal.png background, 7s delay on victory audio before showing menu.
...
def play_win_sequence(secret_word):
    """Sequence for winning: winnormal.png, 7s wait with victory audio."""
    pygame.mixer.music.stop()
    
    # Chargement du background de victoire spécifique
    ...
    # Fondu au noir sur le nouveau BG
    ...
    while True:
        ...
        pygame_utils.draw_button_with_border(screen, rect_retry, constants.DARK_BLUE, constants.DARK_BLUE_HOVER, m_pos, retry_t, fonts["button"])
        pygame_utils.draw_button_with_border(screen, rect_quit, constants.DARK_BLUE, constants.DARK_BLUE_HOVER, m_pos, quit_t, fonts["button"])
...
def play_lose_sequence(secret_word, state):
    ...
    while True:
        ...
        t_tip = fonts["small"].render("Astuce : Au pendu, il ne faut jamais perdre la tête...", True, constants.GOLD)
...
def main():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn_pause_rect.collidepoint(event.pos):
                    ...
                    if rect_cont.collidepoint(event.pos):
                        pygame_utils.play_click_sound(); paused = False
                    if rect_reset.collidepoint(event.pos):
                        pygame_utils.play_click_sound(); game_state, secret_word, timer = initialize_game(); paused = False
                    if rect_quit.collidepoint(event.pos):
                        pygame_utils.play_click_sound(); return_to_main_menu()
            ...
            if not paused and event.type == pygame.KEYDOWN and game_state["status"] == "in_progress":
                ...
                if letter.isalpha() and len(letter) == 1 and letter not in game_state["letters_played"]:
                    ...
                    if game_state["errors"] == old_err: timer += 5
                    else: timer -= 5
        ...
        if game_state["status"] == "in_progress":
            ...
            if paused:
                overlay = pygame.Surface((constants.WIDTH, constants.HEIGHT), pygame.SRCALPHA)
                overlay.fill(constants.BLACK_OVERLAY); screen.blit(overlay, (0, 0))
                txt_pause = fonts["word"].render(language_manager.get_text("hard_pause"), True, constants.GOLD)
